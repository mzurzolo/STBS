...
# "If there is not a file called project.py, then
# copy the file project.backup.py and call the project.py"
# I copy Projects this way so the Project's "starting point" is saved,
# and you can make changes to project.py without worrying about losing the
...
if [ ! -f project.py ] ; then
  cp project.backup.py project.py
fi
